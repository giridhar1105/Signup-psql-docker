...
function Signup() {
  ...
  const [otpError, setOtpError] = useState("");  // Separate state for OTP errors
  const [signupError, setSignupError] = useState("");  // Separate state for Signup errors
  ...
  async function getOtp() {
    ...
    try {
      ...
      setOtpError("An error occurred while sending OTP. Please try again later.");
    } finally {
      setOtpLoading(false);
    }
  }
        ...
        >
          ...
          <form onSubmit={onSignup}>
            ...
            <motion.button
              ...
              className={`bg-red-500 text-white p-3 w-full rounded-md mt-5 ${otpLoading ? "bg-red-300 cursor-not-allowed" : "hover:bg-red-600"} shadow-md transition-transform duration-300`}
            ...
            <motion.button
              type="submit"
              disabled={loading}
              className={`bg-red-500 text-white p-3 w-full rounded-md mt-5 ${loading ? "bg-red-300 cursor-not-allowed" : "hover:bg-red-600"} shadow-md transition-transform duration-300`}
              whileHover={{ scale: 1.05 }}
              transition={{ type: "spring", stiffness: 300 }}
            >
